...
Primero necesitamos un usuario donde no tenga acceso a shell, para esto lo crearemos, tambien necesitamos un grupo al que pertenecera el usuario.
...
Ahora dentro de la carpeta nfs, crearemos una carpeta para el usuario y le daremos a la carpeta el grupo del usuario `sudo chown root:sftp <carpeta.a.la.que.queremos.restringir.los.permisos>` despues tendremos que editar el archivo `/etc/ssh/sshd_config` y añadir al final
